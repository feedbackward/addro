'''Make CIFAR-10/100-C data (training and testing) based on the code of Huang et al. (2023).'''
...
from setup.cifar100 import get_cifar100
...
# https://github.com/zhuohuangai/SharpDRO/blob/main/corruptions/make_cifar_c.py
# https://github.com/zhuohuangai/SharpDRO/blob/main/corruptions/make_cifar100_c.py
...
def make_cifar_c(cifar_10=False, cifar_100=False):
    '''
    Prepare corrupted CIFAR-10/100 data and save to disk.
    '''

    if cifar_10 == False and cifar_100 == False:
        raise ValueError("Choose either cifar_10 or cifar_100.")

    # Number of severity levels is based completely on CIFAR-10/100-C of Hendrycks and Dietterich.
    # 10:  https://zenodo.org/records/2535967
    # 100: https://zenodo.org/records/3555552
    num_severity_levels = 5
    
    # Get the original clean data if needed.
    if cifar_10:
        dsname = "cifar10"
        data_raw_tr, data_raw_te = get_cifar10()
    else:
        dsname = "cifar100"
        data_raw_tr, data_raw_te = get_cifar100()

    # Conversion map.
    convert_img = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                  torchvision.transforms.ToPILImage()])

    # Training data loop.
    for method_name in corruption_dict.keys():
        print("(train) Creating images for corruption of type -- {} --.".format(method_name))
    
        for severity in range(1, num_severity_levels+1):
            cifar_c, labels = [], []
            save_path = os.path.join(data_path, "{}-sharpdro".format(dsname), "train", str(severity))
    
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            
            corruption = lambda clean_img: corruption_dict[method_name](clean_img, severity)
    
            for img, label in zip(data_raw_tr.data, data_raw_tr.targets):
                labels.append(label)
                cifar_c.append(np.uint8(corruption(convert_img(img))))
    
            np.save(os.path.join(save_path, corruption_dict[method_name].__name__ + ".npy"),
                    np.array(cifar_c).astype(np.uint8))
    
            np.save(os.path.join(save_path, "labels.npy"),
                    np.array(labels).astype(np.uint8))
    
    # Test data loop.
    for method_name in corruption_dict.keys():
        print("(test) Creating images for corruption of type -- {} --.".format(method_name))
    
        for severity in range(1, num_severity_levels+1):
            cifar_c, labels = [], []
            save_path = os.path.join(data_path, "{}-sharpdro".format(dsname), "test", str(severity))
            ...
            for img, label in zip(data_raw_te.data, data_raw_te.targets):
                labels.append(label)
                cifar_c.append(np.uint8(corruption(convert_img(img))))
    ...
    return "Made SharpDRO-style CIFAR-{}.".format("10" if cifar_10 else "100")
