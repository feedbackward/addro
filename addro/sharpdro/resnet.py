'''Preparation of Wide ResNet based on the code of Huang et al. (2023).'''
...
# https://github.com/zhuohuangai/SharpDRO/blob/main/models/resnet.py
# Notes:
# For the most part we use their original code as-is, except that model_urls
# is no longer included in torchvision.models.resnet, so we place it directly.


# The model_urls is supposed to come from torchvision.models.resnet, but it has
# been removed from the latest version, so we put it here so everything runs.
model_urls = {
...
# Key ResNet-related class definitions.
